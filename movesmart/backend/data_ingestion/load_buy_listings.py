"""data_ingestion/load_buy_listings.py — Load buy_resident_latest.json into MongoDB listings collection (Architecture.md §5, §6, FR-6)
...
def run_ingestion(json_path: str = "data/buy_resident_latest.json") -> int:
    """Ingest seed buy listings from buy_resident_latest.json into MongoDB `listings` collection."""
    ...
    logger.info(f"Loading buy listings from {json_path}...")
    ...
    for item in data:
        ...
        title = listing.get("title") or entity.get("title") or f"{prop.get('bedrooms', 1)} BHK Property for Sale"
        ...
        doc = {
            ...
            "source": "seed",
            "source_detail": "buy_resident_latest",
            "verification_flags": {
                "is_suspicious": False,
                "checked_at": now
            },
    ...
    if operations:
        ...
        logger.info(f"Ingested buy seed listings: {result.upserted_count} inserted, {result.modified_count} modified.")
        return len(operations)
    return 0


if __name__ == '__main__':
    target_path = sys.argv[1] if len(sys.argv) > 1 else "../data/buy_resident_latest.json"
