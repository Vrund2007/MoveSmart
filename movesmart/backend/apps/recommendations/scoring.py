...
def score_localities(profile: dict, localities: List[dict]) -> List[dict]:
    ...
    # ── SINGLE BATCH API CALL for ALL localities ──────────────────────────────
    ...
    for loc in combined_localities:
        ...
        # Look up commute from batch result (no extra API call)
        ...
        if budget >= avg_rent * 1.15:
            budget_badge = f"💰 Under Budget (Avg Rs.{avg_rent:,}/mo)"
        elif budget >= avg_rent * 0.85:
            budget_badge = f"🎯 Perfect Budget Fit (Avg Rs.{avg_rent:,}/mo)"
        else:
            budget_badge = f"⚠️ Above Budget (Avg Rs.{avg_rent:,}/mo)"

        insights = [
            f"📍 ~{commute_mins:.0f} mins commute to your office in {work_area} ({distance_km} km).",
            f"🏡 Average rent Rs.{avg_rent:,}/mo — {budget_badge.split('(')[0].strip()}.",
            f"🌿 Fits your '{lifestyle_pref.capitalize()}' lifestyle preference with strong livability & subscore metrics."
        ]
        ...
        scored_results.append({
            ...
            "insights": insights,
            "explanation": f"{name} scored {final_score}/100 based on your Rs.{int(budget):,} budget, {commute_mins:.0f}-min commute to {work_area}, and {lifestyle_pref} preference."
        })
...
def _score_residential(kb: dict, budget: float, avg_rent: float) -> float:
    ...
    if avg_rent <= budget:
        # Reward under-budget localities
        savings_ratio = (budget - avg_rent) / budget
        return min(98.0, base + (savings_ratio * 15.0))
    else:
        # Penalize over-budget localities relative to excess rent
...
def _score_business(kb: dict, work_area: str, name: str, commute_mins: float) -> float:
    ...
    if work_area.lower() in name.lower() or name.lower() in work_area.lower():
        return 98.0
    # Linearly scale business/office score with commute minutes
    if commute_mins <= 10:
...
def _score_transit(commute_mins: float, commute_tolerance: float) -> float:
    if commute_mins <= commute_tolerance:
        # Scale transit score from 96 down to 75 within tolerance
        return max(75.0, 96.0 - ((commute_mins / commute_tolerance) * 21.0))
    else:
        # Penalize extra commute time past tolerance
